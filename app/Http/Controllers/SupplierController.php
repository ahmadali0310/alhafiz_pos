...
//    new
    ...
    {
        $lims_deposit_list = SupplierDeposit::where('supplier_id', $id)->get();
        $deposit_id = [];
        $deposits = [];
...
//        return $data;
        ...
        if($data['amount'] > 0) {
            $lims_supplier_data->deposit += $data['amount'];
            $lims_supplier_data->save();
        }


        if($data['remaining'] > 0) {
            $lims_supplier_data->deposit -= $data['remaining'];
            $lims_supplier_data->save();
        }

        SupplierDeposit::create($data);
        $message = 'Data inserted successfully';

        return redirect('supplier')->with('create_message', $message);
    }
