...
//            $revenue = SaleRevenue::whereDate('date', '>=' , $start_date)->whereDate('date', '<=' , $end_date)->where('user_id', Auth::id())->sum('revenue');
//            $return = ReturnRevenue::whereDate('date', '>=' , $start_date)->whereDate('date', '<=' , $end_date)->where('user_id', Auth::id())->sum('return_benifit');
//            $expense = ExpenseTotal::whereDate('date', '>=' , $start_date)->whereDate('date', '<=' , $end_date)->where('user_id', Auth::id())->sum('total_expence');
...
//            $recent_payment = Payment::orderBy('id', 'desc')->where('user_id', Auth::id())->take(5)->get();
            ...
            $revenue = SaleRevenue::whereDate('date', '>=' , $start_date)->whereDate('date', '<=' , $end_date)->sum('revenue');
            $return = ReturnRevenue::whereDate('date', '>=' , $start_date)->whereDate('date', '<=' , $end_date)->sum('return_benifit');
            $expense = ExpenseTotal::whereDate('date', '>=' , $start_date)->whereDate('date', '<=' , $end_date)->sum('total_expence');
//            $revenue -= $return;
            $profit = ($revenue - $expense) - $return;
