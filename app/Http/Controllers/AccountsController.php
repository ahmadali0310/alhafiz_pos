    ...
    public function balanceSheet()
    {
        ...
        if($role->hasPermissionTo('balance-sheet')){
            ...
            foreach ($lims_account_list as $account) {
                $credit_change = Payment::whereNotNull('sale_id')->orWhere('customer_id', '!=', 'NULL')->where('account_id', $account->id)->sum('change');
                $payment_recieved = Payment::whereNotNull('sale_id')->orWhere('customer_id', '!=', 'NULL')->where('account_id', $account->id)->sum('amount');
                // $payment_recieved = $payment_recieved + $credit_change;
                
                $debit_change = Payment::whereNotNull('purchase_id')->orWhere('seller_id', '!=', 'NULL')->where('account_id', $account->id)->sum('amount');
                $payment_sent = Payment::whereNotNull('purchase_id')->orWhere('seller_id', '!=', 'NULL')->where('account_id', $account->id)->sum('change');
                // $payment_sent = $payment_sent - $debit_change;
                ...
                $credit[] = $payment_recieved + $debit_change + $return_purchase + $account->initial_balance;
                $debit[] = $payment_sent + $returns + $expenses + $payrolls + $credit_change;
...
//        $flag = true;
        if($data['type'] == '0' || $data['type'] == '2'){
            $credit_list = Payment::whereNotNull('sale_id')->orWhere('customer_id', '!=', 'NULL')->where('account_id', $data['account_id'])->whereDate('created_at', '>=' , $data['start_date'])->whereDate('created_at', '<=' , $data['end_date'])->get();
        }
        if($data['type'] == '0' || $data['type'] == '1'){
            $debit_list = Payment::whereNotNull('purchase_id')->orWhere('seller_id', '!=', 'NULL')->where('account_id', $data['account_id'])->whereDate('created_at', '>=' , $data['start_date'])->whereDate('created_at', '<=' , $data['end_date'])->get();
