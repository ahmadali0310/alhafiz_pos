...
use App\ExpenseTotal;
use App\ReturnRevenue;
use App\SaleRevenue;
use App\StockReport;
...
class ReportController extends Controller
{
    ...
    public function profitLoss(Request $request)
    {
        ...
        $revenue1 = SaleRevenue::whereDate('date', '>=' , $start_date)->whereDate('date', '<=' , $end_date)->sum('revenue');
        $return1 = ReturnRevenue::whereDate('date', '>=' , $start_date)->whereDate('date', '<=' , $end_date)->sum('return_benifit');
        $expense1 = ExpenseTotal::whereDate('date', '>=' , $start_date)->whereDate('date', '<=' , $end_date)->sum('total_expence');
        $revenue1 -= $return1;
        $profit = $revenue1 - $expense1;
        ...
        return view('report.profit_loss', compact('profit','revenue1','return1','expense1','purchase', 'total_purchase', 'sale', 'total_sale', 'return', 'purchase_return', 'total_return', 'total_purchase_return', 'expense', 'payroll', 'total_expense', 'total_payroll', 'payment_recieved', 'payment_recieved_number', 'cash_payment_sale', 'cheque_payment_sale', 'credit_card_payment_sale', 'gift_card_payment_sale', 'paypal_payment_sale', 'deposit_payment_sale', 'payment_sent', 'payment_sent_number', 'cash_payment_purchase', 'cheque_payment_purchase', 'credit_card_payment_purchase', 'warehouse_name', 'warehouse_sale', 'warehouse_purchase', 'warehouse_return', 'warehouse_purchase_return', 'warehouse_expense', 'start_date', 'end_date'));
    }
...
//        dd($product_name);
    ...
    public function stockReport(Request $request) {
        ...
        $users = StockReport::where('warehouse_id', $warehouse_id)->get();

//        return $users;
        return view('report.stock_report', compact('warehouse_id', 'lims_warehouse_list', 'users'));
    }
    ...
    public function customerReport(Request $request)
    {
        ...
        $lims_payment_data = DB::table('payments')
                        ...
                        //   ->where('sales.customer_id', $customer_id)
                           ->whereDate('payments.created_at', '>=' , $start_date)
                           ->whereDate('payments.created_at', '<=' , $end_date)
                           ->select('payments.*', 'payments.payment_reference as sale_reference')
    ...
    public function supplierReport(Request $request)
    {
        ...
        $lims_payment_data = DB::table('payments')
                        //   ->join('purchases', 'payments.purchase_id', '=', 'purchases.id')
                           ->where('payments.seller_id', $supplier_id)
                           ->whereDate('payments.created_at', '>=' , $start_date)
                           ->whereDate('payments.created_at', '<=' , $end_date)
                           ->select('payments.*', 'payments.payment_reference as purchase_reference')
                           ->orderBy('payments.created_at', 'desc')
                           ->get();
