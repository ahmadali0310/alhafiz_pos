...
class PurchaseController extends Controller
{
    ...
    public function getProduct($id)
    {
        $lims_product_warehouse_data = DB::table('products')
            ->select('products.id', 'products.code', 'products.name', 'products.type')
            ->where([
        ...
        return [$product_code, $product_name , $product_type];
    }
...
//            dd($i);
            ...
            if ($lims_product_warehouse_data) {
                ...
                $lims_product_warehouse_data->size = $size[$i];
            } 
            else {
        ...
        foreach ($lims_product_purchase_data as $product_purchase_data) {
            ...
            if($lims_product_data->is_variant) {
                ...
                $lims_product_warehouse_data = Product_Warehouse::where([
                    ...
                    ['size', $product_purchase_data->size],
...
//            dd($product_purchase_data);
            ...
            $lims_product_warehouse_data->size -= $product_purchase_data->size;
            $lims_product_warehouse_data->save();
//            $lims_product_warehouse_data->delete();
        ...
        foreach ($product_id as $key => $pro_id) {
            ...
            if($lims_product_warehouse_data){
                $old =  $lims_product_warehouse_data->qty;
                $lims_product_warehouse_data->delete();
                $lims_product_warehouse_data = new Product_Warehouse();
                ...
                if($lims_product_data->is_variant)
                    $lims_product_warehouse_data->variant_id = $lims_product_variant_data->variant_id;
                $lims_product_warehouse_data->warehouse_id = $data['warehouse_id'];
                $lims_product_warehouse_data->qty += ($new_recieved_value + $old);
                $lims_product_warehouse_data->size = $size[$key];
                $lims_product_warehouse_data->save();
//
            }
            else {
...
//            DB::query("DELETE FROM product_warehouse WHERE size = 0 AND qty = 0");
            DB::statement("DELETE FROM product_warehouse WHERE size = 0 AND qty = 0");
...
//            DB::delete()
            ...
            $product_purchase['qty'] = $qty[$key];
            $product_purchase['size'] = $size[$key];
    ...
    public function addPayment(Request $request)
    {
        ...
        if($balance > 0 || $balance < 0)
            $lims_purchase_data->payment_status = 3;
        elseif ($balance == 0)
