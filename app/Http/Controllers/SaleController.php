            ...
            if($sale_unit[$i] != 'n/a') {
                ...
                if($data['sale_status'] == 1){
                    ...
                    if($request->has('is_pos')){
                        if($quantity >= $size[$i]){
                            $saleReminder = $quantity % $size[$i];
                            $mainQty = $quantity / $size[$i];
                            $mainQty = (double)$mainQty;
                            $saleReminder = (double)$saleReminder;
                            $saleQty = $saleReminder;
//                            $finalQty = (double)$mainQty.'.'.$saleQty;
                            $finalQty = (double)$mainQty+$saleQty;
//                            return dd($mainQty);
                            $lims_product_data->qty = $lims_product_data->qty - $finalQty;
                        } else{
//                            $finalQty = (double)$quantity;
//                            $finalQty = '0.'.$quantity;
//                            return dd($quantity);
                            $finalQty = $quantity;
                            $lims_product_data->qty = $lims_product_data->qty - $finalQty;
                    ...
                    if($isSubQty == 1){
                        if($request->has('is_pos')){
                            $oldSubQty = (double)$lims_product_warehouse_data->sub_qty;
                            $oldQty = (double)$lims_product_warehouse_data->qty;
                            $oldSize = (double)$lims_product_warehouse_data->size;
                            $saleQty = (double)$quantity + $oldSubQty;
                            $saleReminder = 0;
                            $mainQty = 0;
                            if($saleQty >= $size[$i]){
                                $saleReminder = $saleQty % $size[$i];
                                $mainQty = $saleQty / $size[$i];
                                $mainQty = (int)$mainQty;
                                $saleReminder = (int)$saleReminder;
                                $saleQty = $saleQty - $size[$i];
                            }
                    ...
                    $s = strval($lims_product_warehouse_data->sub_qty);

                    $lims_product_warehouse_data->sub_qty = $s;
...
//                        'warehouse_id' => $data['warehouse_id']
//                    ])->get();
//                    return dd($lims_product_warehouse_datas);
                }
            }
            ...
            if($request->has('is_pos')){
                $product_sale['qty'] = $mail_data['qty'][$i] = '0.'.$qty[$i];
            } else{
            ...
            $product_sale['net_unit_cost'] = $net_cost_price[$i];
            $product_sale['total_cost'] = $cost_total[$i];
...
//            dd($product_sale);
        ...
        if($lims_sale_data->sale_status == '1')
            if($request->has('is_pos'))
                return redirect('sales/gen_invoice_pos/' . $lims_sale_data->id)->with('message', $message);
            else
...
//        return $lims_product_warehouse_data;
        ...
        $qty_list = array_unique($qty_list);

        $product_data = [$product_code, $product_name, $product_qty, $product_type, $product_id, $product_list, $qty_list, $product_cost];
...
//            dd($lims_sale_data);
    ...
    public function genInvoice($id)
    {
        ...
        $lims_unit = DB::select('select units.unit_name from units join product_sales on units.id = product_sales.sale_unit_id where product_sales.sale_id ='.$id);
        $s = $lims_unit[0]->unit_name;
        ...
        foreach ($lims_product_sale_data as $value) {
            array_push($total_qty, $value->qty);
        }

        $sum_total_qty = array_sum($total_qty);


        return view('sale.invoice', compact('s','lims_sale_data','sum_total_qty', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));
    }
    public function genInvoicePos($id)
    {
        $lims_sale_data = Sale::find($id);
        $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
        $lims_biller_data = Biller::find($lims_sale_data->biller_id);
        ...
        $sum_total_qty = sizeof($total_qty);
    ...
    public function destroy($id)
    {
        $url = url()->previous();
        $lims_sale_data = Sale::find($id);
        $sRef = explode('-', $lims_sale_data->reference_no);
                ...
                if($sRef[0] == 'posr'){
                    ...
                    if($saleQty <= 0){
                        ...
                        if($saleQty >= $product_sale->size){
                            ...
                            $saleQty = $product_sale->size - $saleQty;
                            $saleQty = (int)$saleQty;
                        }
                    } 
