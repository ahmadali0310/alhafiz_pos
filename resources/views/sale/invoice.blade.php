    ...
    <div id="receipt-data">
        ...
        <table>
            <tbody>
                ...
                @endphp
                <tr><td colspan="2">{{$product_name}}<br>{{$product_sale_data->qty}} x {{$product_sale_data->size}} = {{$product_sale_data->qty * $product_sale_data->size}} x {{$product_sale_data->net_unit_price}} </td>
{{--                    <br>{{$product_sale_data->qty}} x {{number_format((float)($product_sale_data->total / $product_sale_data->qty), 2, '.', '')}}--}}
            ...
            <tr>
                ...
                <th style="text-align:right">{{number_format((float)$sum_total_qty, 1, '.', '')}}</th>
            </tr>
                <tr>
                ...
                @if($lims_sale_data->discount_status == 1)
                <tr>
                ...
                <tr>
                    ...
                    <th style="text-align:right">{{number_format($lims_sale_data->paid_amount , 2, '.','')}}</th>
                </tr>

                <tr>
                    <th colspan="2">Current Remaining</th>
                    <th style="text-align:right">{{number_format(($lims_sale_data->grand_total - $lims_sale_data->paid_amount), 2, '.','')}}</th>
                </tr>

                <tr>
                    <th colspan="2">Previous Remaining</th>
                    <th style="text-align:right">{{number_format($lims_customer_data->deposit - ($lims_sale_data->grand_total - $lims_sale_data->paid_amount) , 2, '.','')}}</th>
                </tr>

                <tr>
                    <th colspan="2">Total Remaining</th>
                    <th style="text-align:right">{{number_format($lims_customer_data->deposit, 2, '.', '')}}</th>
                </tr>
                <tr>
        ...
        <div class="centered" style="margin:30px 0 50px">
            ...
            {{trans('file.Developed By')}} Khyber Coded</strong></small>
        </div>
    </div>
</div>
